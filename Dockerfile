...
RUN CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o sprint_12_tracker_app .
...
COPY --from=builder --chown=appuser:appgroup /app/sprint_12_tracker_app .

USER appuser

ENTRYPOINT ["./sprint_12_tracker_app"]